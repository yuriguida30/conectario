...
export const AdminDashboard: React.FC<{ currentUser: User; onNavigate: (page: string, params?: any) => void; onLogout: () => void }> = ({ currentUser, onNavigate, onLogout }) => {
  ...
  const addMenuSection = () => {
      ...
      const sections = myBusiness.menu || [];
      setMyBusiness({
          ...myBusiness,
          menu: [...sections, { title: 'Nova Seção', items: [] }]
      });
  };

  const addMenuItem = (sIdx: number) => {
      if (!myBusiness || !myBusiness.menu) return;
      const menu = [...myBusiness.menu];
      menu[sIdx].items.push({
          id: Math.random().toString(36).substring(2, 9),
          name: 'Novo Item',
          price: 0,
          description: ''
      });
      setMyBusiness({ ...myBusiness, menu });
  };
  ...
  return (
    <div className="pb-24 pt-8 md:pt-20 px-4 max-w-6xl mx-auto">
      ...
      <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100">
          <div className="flex items-center gap-4">
              ...
              <div>
                  ...
                  <p className="text-xs text-slate-500 font-medium">Painel Administrativo Premium</p>
              </div>
          </div>
          ...
          <div className="space-y-8 animate-in fade-in">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                      ...
                      <h3 className="text-2xl font-black text-ocean-950">{coupons.length}</h3>
                  </div>
                  <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                  ...
                  <button onClick={() => setView('COUPONS')} className="bg-white p-6 rounded-3xl border border-slate-100 flex flex-col items-center justify-center gap-2 group hover:border-ocean-300">
                      <Ticket size={24} className="text-ocean-600" /><span className="text-xs font-bold text-ocean-950">Meus Cupons</span>
                  </button>
      ...
      {view === 'GALLERY' && myBusiness && (
          <div className="space-y-6 animate-in slide-in-from-left-4">
              ...
              <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 space-y-6">
                  ...
                  <button onClick={handleSaveBusiness} className="w-full bg-ocean-600 text-white font-bold py-4 rounded-2xl">SALVAR GALERIA</button>
              </div>
          </div>
      )}
      ...
      {/* VIEW PERFIL - MANTIDA E MELHORADA */}
              ...
              <div className="bg-white w-full max-w-xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95">
                  ...
                  <form onSubmit={handleSaveCoupon} className="p-8 space-y-4 max-h-[80vh] overflow-y-auto">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                          <input type="number" step="0.01" className="w-full border-slate-200 rounded-xl p-3 bg-slate-50" placeholder="Preço Original" value={currentCoupon.originalPrice} onChange={e => setCurrentCoupon({...currentCoupon, originalPrice: Number(e.target.value)})} />
                          <input type="number" step="0.01" className="w-full border-slate-200 rounded-xl p-3 bg-slate-50" placeholder="Com Desconto" value={currentCoupon.discountedPrice} onChange={e => setCurrentCoupon({...currentCoupon, discountedPrice: Number(e.target.value)})} />
                      </div>
                      <input required className="w-full border-slate-200 rounded-xl p-3 bg-slate-50 font-mono text-center text-lg uppercase" value={currentCoupon.code} onChange={e => setCurrentCoupon({...currentCoupon, code: e.target.value.toUpperCase()})} placeholder="CODIGO10" />
                      <button type="submit" className="w-full bg-ocean-600 text-white font-bold py-4 rounded-2xl">PUBLICAR CUPOM</button>
                  </form>
              </div>
          </div>
      )}
