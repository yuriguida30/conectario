      ...
      <div className="flex-1 p-6 md:p-12 overflow-y-auto">
          ...
          {activeTab === 'BRANDING' && (
              <div className="max-w-2xl">
                  <h1 className="text-2xl font-bold text-ocean-950 mb-6">Identidade Visual & Marca</h1>
                  <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-lg">
                      <form onSubmit={handleSaveBranding} className="space-y-6">
                          <div className="grid md:grid-cols-2 gap-6">
                              <div>
                                  <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Nome Principal</label>
                                  <input 
                              ...
                              <div>
                                  <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Destaque (Cor)</label>
                                  <input 
                                    className="w-full border rounded-lg p-3 font-bold text-lg text-gold-500" 
                                    value={appConfig.appNameHighlight}
                                    onChange={e => setAppConfig({...appConfig, appNameHighlight: e.target.value})}
                                    placeholder="RIO"
                                  />
                              </div>
                          </div>
                          ...
                          <div className="space-y-6">
                              <ImageUpload 
                                  label="Logo da Barra de Navegação (Pequeno)"
                                  currentImage={appConfig.logoUrl}
                                  onImageSelect={(base64) => setAppConfig({...appConfig, logoUrl: base64})}
                                  className="w-32"
                              />
                              <ImageUpload 
                                  label="Logo de Login & Inscrição (Grande)"
                                  currentImage={appConfig.loginLogoUrl}
                                  onImageSelect={(base64) => setAppConfig({...appConfig, loginLogoUrl: base64})}
                                  className="w-48"
                              />
                              <ImageUpload 
                                  label="Favicon (Ícone da Aba)"
                                  currentImage={appConfig.faviconUrl}
                                  onImageSelect={(base64) => setAppConfig({...appConfig, faviconUrl: base64})}
                                  className="w-16"
                              />
                          </div>

                          <button type="submit" className="w-full bg-ocean-600 text-white font-bold py-3 rounded-xl hover:bg-ocean-700 transition-colors shadow-lg">
                              Salvar Identidade
                          </button>
                      </form>
                  </div>
          ...
          {activeTab === 'FEATURED' && (
              <div className="max-w-2xl">
                  <h1 className="text-2xl font-bold text-ocean-950 mb-6">Editar Destaque Premium (Home)</h1>
                  <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-lg">
                      <form onSubmit={handleSaveFeatured} className="space-y-4">
                          ...
                          <div>
                              <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Subtítulo / Descrição</label>
                              <textarea 
                                className="w-full border rounded-lg p-3" 
                                rows={3}
                                value={featuredConfig.subtitle}
                                onChange={e => setFeaturedConfig({...featuredConfig, subtitle: e.target.value})}
                              />
                          </div>
                          <div>
                              <ImageUpload 
                                  label="Imagem de Fundo do Destaque"
                                  currentImage={featuredConfig.imageUrl}
                                  onImageSelect={(base64) => setFeaturedConfig({...featuredConfig, imageUrl: base64})}
                              />
                          </div>
                          <div>
                          ...
                          <button type="submit" className="w-full bg-ocean-600 text-white font-bold py-3 rounded-xl hover:bg-ocean-700 transition-colors">
                              Salvar Alterações
                          </button>
                      </form>
                  </div>
                   ...
                   {editingUser ? (
                       <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-lg mb-6 animate-in fade-in">
                           <div className="flex justify-between items-center mb-4">
                               <h3 className="text-lg font-bold text-ocean-900">Editar Permissões: {editingUser.name}</h3>
                               <button onClick={() => setEditingUser(null)}><X size={20} className="text-slate-400"/></button>
                           </div>
                           
                           <div className="space-y-4">
                               <div className="grid md:grid-cols-2 gap-4">
                                   <div>
                                       <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Função</label>
                                       <select 
                                   ...
                                   {editingUser.role === UserRole.COMPANY && (
                                       <div>
                                            <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Limite de Cupons Ativos</label>
                                            <input 
                                                type="number" 
                                                className="w-full border rounded-lg p-2" 
                                                value={editingUser.maxCoupons || 0}
                                                onChange={e => setEditingUser({...editingUser, maxCoupons: parseInt(e.target.value)})}
                                            />
                                       </div>
                                   )}
                               </div>
                                   ...
                                   <div className="bg-slate-50 p-4 rounded-xl space-y-3">
                                       <label className="flex items-center gap-3 cursor-pointer">
                                           <input 
                                                ...
                                                checked={editingUser.permissions?.canCreateCoupons || false}
                                                onChange={e => setEditingUser({
                                                    ...editingUser, 
                                                    permissions: { ...editingUser.permissions!, canCreateCoupons: e.target.checked }
                                                })}
                                           />
                                           <span className="text-sm font-medium text-slate-700">Pode Criar Cupons</span>
                                       </label>
                                       <label className="flex items-center gap-3 cursor-pointer">
                                           <input 
                                                type="checkbox" 
                                                className="w-5 h-5 text-ocean-600 rounded"
                                                checked={editingUser.permissions?.canManageBusiness || false}
                                                onChange={e => setEditingUser({
                                                    ...editingUser, 
                                                    permissions: { ...editingUser.permissions!, canManageBusiness: e.target.checked }
                                                })}
                                           />
                                           <span className="text-sm font-medium text-slate-700">Pode Ter Página no Guia</span>
                                       </label>
                                   </div>
                               )}
                               
                               <button onClick={handleUpdateUserPermissions} className="bg-ocean-600 text-white font-bold py-2 px-6 rounded-lg hover:bg-ocean-700">
                                   Salvar Alterações
                               </button>
                           </div>
                       </div>
                                      ...
                                      <tr key={u.id} className="hover:bg-slate-50 transition-colors">
                                          ...
                                          <td className="p-4">
                                              <span className={`px-2 py-1 rounded text-xs font-bold ${
                                                  ...
                                                  u.role === UserRole.COMPANY ? 'bg-blue-100 text-blue-700' :
                                          ...
                                          <td className="p-4">
                                              {u.role === UserRole.COMPANY && (
                                                  <div className="flex flex-col gap-1 text-xs">
                                                      <span className={u.permissions?.canCreateCoupons ? 'text-green-600' : 'text-red-400'}>
                                                          {u.permissions?.canCreateCoupons ? 'Cupons: SIM' : 'Cupons: NÃO'}
                                                          {u.permissions?.canCreateCoupons && ` (Max: ${u.maxCoupons})`}
                                                      </span>
                                                      <span className={u.permissions?.canManageBusiness ? 'text-green-600' : 'text-red-400'}>
                                                          {u.permissions?.canManageBusiness ? 'Guia: SIM' : 'Guia: NÃO'}
                                                      </span>
                                                  </div>
                                              )}
                                          </td>
          ...
          {activeTab === 'COLLECTIONS' && (
              <div className="max-w-6xl">
                  {/* ... Keep existing COLLECTIONS content ... */}
                  <div className="flex justify-between items-center mb-6">
                      <h1 className="text-2xl font-bold text-ocean-950">Coleções & Listas</h1>
                      <button 
                        onClick={() => setEditingCollection({ title: '', description: '', businessIds: [] })}
                        className="bg-ocean-600 text-white font-bold py-2 px-4 rounded-xl flex items-center gap-2 shadow-lg"
                      >
                          <Plus size={20} /> Nova Coleção
                      </button>
                  </div>

                  {editingCollection ? (
                      <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-xl animate-in fade-in">
                          <h3 className="text-lg font-bold mb-4">{editingCollection.id ? 'Editar Coleção' : 'Nova Coleção'}</h3>
                          <form onSubmit={handleSaveCollection} className="space-y-6">
                              <div className="grid md:grid-cols-2 gap-6">
                                  <div className="space-y-4">
                                      <div>
                                          <label className="block text-xs font-bold uppercase mb-1">Título da Coleção</label>
                                          <input 
                                      ...
                                      <div>
                                          <ImageUpload 
                                              ...
                                              currentImage={editingCollection.coverImage}
                                              onImageSelect={(base64) => setEditingCollection({...editingCollection, coverImage: base64})}
                                          />
                                      </div>
                                  </div>

                                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                                      <h4 className="font-bold text-sm mb-2">Adicionar Empresas</h4>
                                      <div className="relative mb-2">
                                          <Search size={16} className="absolute left-3 top-2.5 text-slate-400" />
                                          <input 
                                              type="text" 
                                              placeholder="Filtrar empresas..." 
                                              className="w-full pl-9 p-2 rounded-lg border text-sm"
                                      ...
                                      <div className="h-60 overflow-y-auto space-y-2 pr-2">
                                          {allBusinesses
                                            .filter(b => b.name.toLowerCase().includes(collectionSearch.toLowerCase()))
                                            .map(biz => (
                                              <label key={biz.id} className={`flex items-center gap-3 p-2 rounded-lg cursor-pointer transition-colors ${editingCollection.businessIds?.includes(biz.id) ? 'bg-ocean-100 border border-ocean-200' : 'bg-white border border-slate-100'}`}>
                                                  <input 
                                                      type="checkbox"
                                                      className="rounded text-ocean-600 focus:ring-ocean-500 w-4 h-4"
                                                      checked={editingCollection.businessIds?.includes(biz.id) || false}
                                                      onChange={() => toggleBusinessInCollection(biz.id)}
                                                  />
                                                  <div className="flex-1">
                                                      <div className="text-sm font-bold text-slate-800">{biz.name}</div>
                                      ...
                                      <div className="mt-2 text-xs text-right text-slate-500">
                                          {editingCollection.businessIds?.length} selecionados
                                      </div>
                                  </div>
                              </div>
                              
                              <div className="flex gap-3 pt-4 border-t border-slate-100">
                                  <button type="button" onClick={() => setEditingCollection(null)} className="px-6 py-2 rounded-lg border hover:bg-slate-50 font-bold text-slate-600">Cancelar</button>
                                  <button type="submit" className="px-8 py-2 rounded-lg bg-ocean-600 text-white font-bold hover:bg-ocean-700 shadow-lg">Salvar Coleção</button>
                              </div>
                          </form>
                      </div>
                  ) : (
                      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                          {collections.map(col => (
                              <div key={col.id} className="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm hover:shadow-lg transition-all group">
                                  <div className="h-40 relative">
                                      <img src={col.coverImage} className="w-full h-full object-cover" />
                                      <div className="absolute inset-0 bg-black/40 group-hover:bg-black/20 transition-colors"></div>
                                      <h3 className="absolute bottom-4 left-4 text-white font-bold text-xl shadow-black drop-shadow-md">{col.title}</h3>
                                  </div>
                                  <div className="p-4">
                                      <p className="text-sm text-slate-600 mb-4 line-clamp-2">{col.description}</p>
                                      <div className="flex justify-between items-center text-xs text-slate-500">
                                          <span className="bg-slate-100 px-2 py-1 rounded font-bold">{col.businessIds.length} lugares</span>
                                          <div className="flex gap-2">
                                              <button onClick={() => setEditingCollection(col)} className="text-ocean-600 hover:text-ocean-800 p-1"><Edit size={16}/></button>
                                              <button onClick={() => handleDeleteCollection(col.id)} className="text-red-400 hover:text-red-600 p-1"><Trash2 size={16}/></button>
                                          </div>
                                      </div>
                                  </div>
                              </div>
          ...
          {activeTab === 'BLOG' && (
              <div className="max-w-6xl">
                  ...
                  <div className="flex justify-between items-center mb-6">
                      <h1 className="text-2xl font-bold text-ocean-950">Gestão de Blog & Dicas</h1>
                      <button 
                        onClick={() => setEditingPost({ title: '', excerpt: '', content: '' })}
                        className="bg-ocean-600 text-white font-bold py-2 px-4 rounded-xl flex items-center gap-2"
                      >
                  ...
                  {editingPost ? (
                      <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-lg animate-in fade-in">
                          <h3 className="text-lg font-bold mb-4">{editingPost.id ? 'Editar Postagem' : 'Nova Postagem'}</h3>
                          <form onSubmit={handleSavePost} className="space-y-4">
                              ...
                              <div className="grid md:grid-cols-2 gap-4">
                                  <div>
                                      ...
                                      <select 
                                        ...
                                        value={editingPost.category || 'Dica'}
                                  ...
                                  <div>
                                      <ImageUpload 
                                          ...
                                          currentImage={editingPost.imageUrl}
                                          onImageSelect={(base64) => setEditingPost({...editingPost, imageUrl: base64})}
                                      />
                                  </div>
                              </div>
                              <div>
                                  <label className="block text-xs font-bold uppercase mb-1">Resumo (Excerpt)</label>
                                  <textarea 
                                    className="w-full border p-3 rounded-lg" 
                                    rows={2}
                              ...
                              <div className="flex gap-3 pt-2">
                                  <button type="button" onClick={() => setEditingPost(null)} className="px-6 py-2 rounded-lg border hover:bg-slate-50">Cancelar</button>
                                  <button type="submit" className="px-6 py-2 rounded-lg bg-green-600 text-white font-bold hover:bg-green-700">Salvar Postagem</button>
                              </div>
                          </form>
                      </div>
                  ) : (
                      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                          {posts.map(post => (
                              <div key={post.id} className="bg-white rounded-xl border border-slate-100 overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                                  <img src={post.imageUrl} className="h-40 w-full object-cover" />
                                  <div className="p-4">
                                      <span className="text-xs font-bold text-ocean-600 bg-ocean-50 px-2 py-0.5 rounded">{post.category}</span>
                                      <h3 className="font-bold text-ocean-950 mt-2 line-clamp-1">{post.title}</h3>
                                      ...
                                      <div className="flex justify-end gap-2 mt-4 pt-3 border-t border-slate-50">
                                          <button onClick={() => setEditingPost(post)} className="text-ocean-600 p-1 hover:bg-ocean-50 rounded"><Edit size={16}/></button>
                                          <button onClick={() => handleDeletePost(post.id)} className="text-red-400 p-1 hover:bg-red-50 rounded"><Trash2 size={16}/></button>
                                      </div>
                                  </div>
                              </div>
                          ))}
          ...
          {activeTab === 'SUPPORT' && (
              <div className="max-w-4xl">
                  <h1 className="text-2xl font-bold text-ocean-950 mb-6">Mensagens de Suporte</h1>
                  <div className="space-y-4">
                      {supportMessages.length === 0 ? (
                          <div className="text-center py-10 text-slate-400">Nenhuma mensagem.</div>
                      ) : supportMessages.map(msg => (
                          <div key={msg.id} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
                              <div className="flex justify-between items-start mb-2">
                                  <div>
                                      <h3 className="font-bold text-ocean-900">{msg.userName}</h3>
                                      <p className="text-xs text-slate-500">{msg.userEmail}</p>
                                  </div>
                                  <span className="text-xs text-slate-400">{new Date(msg.date).toLocaleDateString()}</span>
                              </div>
                              <p className="text-slate-700 text-sm bg-slate-50 p-4 rounded-xl">{msg.message}</p>
                          </div>
                      ))}
                  </div>
              </div>
          ...
          {activeTab === 'SETTINGS' && (
              <div className="max-w-6xl grid md:grid-cols-2 gap-8 pb-12">
                  ...
                  <div>
                      <h2 className="text-xl font-bold text-ocean-950 mb-4 flex items-center gap-2">
                          <Map size={20} className="text-ocean-500" /> Locais
                      </h2>
                      <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
                          <div className="flex gap-2 mb-4">
                              <input 
                              ...
                              />
                              <button onClick={handleAddLocation} className="bg-ocean-600 hover:bg-ocean-700 text-white px-4 rounded-lg"><Plus size={20}/></button>
                          </div>
                          <div className="space-y-2">
                              {locations.map(loc => (
                                  <div key={loc.id} className="flex justify-between items-center bg-slate-50 border border-slate-200 px-4 py-3 rounded-xl">
                                      <span className="font-medium text-ocean-900 text-sm">{loc.name}</span>
